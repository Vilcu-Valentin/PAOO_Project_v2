...
public class QuestService {
    ...
    private int calculateSuccessProbability(Quest quest) {
        ...
        double[] weights = quest.getQuestType().getStatWeights();

        double weightedStatSum = playerStats.getHealth() * weights[0]
                + playerStats.getMana() * weights[1]
                + playerStats.getStrength() * weights[2]
                + playerStats.getIntelligence() * weights[3]
                + playerStats.getAgility() * weights[4]
                + playerStats.getDefense() * weights[5]
                + playerStats.getDexterity() * weights[6];

        double levelFactor = Math.exp(-(quest.getLevel() + quest.getDifficulty() * 5) / 8.0) * 5;

        double successProbability = weightedStatSum * levelFactor;
        return (int) Math.max(1, Math.min(99, successProbability));
    }
